...
class MchAnim_class:
    ...
    def __repr__(self):
        return "frameCount:{} boneCount:{} oneAddress:{}\n".format(
            self.name, hex(self.frameCount), hex(self.boneCount), hex(self.oneAddress)
        )
...
class MchAlone_class:
    ...
    def __repr__(self):
        return (
            "name{} address:{} size:{} hasTim:{} timoffset:{} ModelOffset:{}\n".format(
                self.name,
                hex(self.Address),
                hex(self.Size),
                hex(self.hasTim),
                hex(self.TimOffset),
                hex(self.ModelOffset),
                hex(self.AnimCount),
            )
        )
...
class MchSkin_class:
    ...
    def __repr__(self):
        return "name: {} vFirst:{} vCount:{} bone:{} fCount:{}\n".format(
            self.name,
            hex(self.vertexFirst),
            hex(self.vertexCount),
            hex(self.bone),
            hex(self.faceCount),
        )
...
class CLUT_class:
    ...
    def __repr__(self):  # print
        return "Size:{} bytes  x:{} y:{} pixH:{} pixV:{}\n".format(
            self.imagesize, self.x, self.y, self.pixH, self.pixV
        )
    ...
    for i in range(len(bm.faces)):
        ...
        if len(f.loops) == 3:
            f.loops[0][uv_layer].uv = (
                uvs[faces[i].vt1].u / 256,
                uvs[faces[i].vt1].v / 256,
            )
            f.loops[1][uv_layer].uv = (
                uvs[faces[i].vt2].u / 256,
                uvs[faces[i].vt2].v / 256,
            )
            f.loops[2][uv_layer].uv = (
                uvs[faces[i].vt3].u / 256,
                uvs[faces[i].vt3].v / 256,
            )
        elif len(f.loops) == 4:
            f.loops[0][uv_layer].uv = (
                uvs[faces[i].vt1].u / 256,
            ...
            f.loops[3][uv_layer].uv = (
                uvs[faces[i].vt4].u / 256,
                uvs[faces[i].vt4].v / 256,
            )
...
def ReadBone(inputfile):
    ...
    for i in range(0, header.BoneCount):
        ...
        # get parent ( 2bytes)
        bone.parent = (
            int.from_bytes(inputfile.read(2), byteorder="little") - 1
        )  # base 1 in mch
        # skip 6 bytes
        inputfile.seek(6, 1)
...
def RestPose(mchfile, boneList, char_name):
    ...
    for i in range(0, header.BoneCount):
        ...
        if i == 0:
            ...
            Vec.rotate(eul)
            boneList[i].tail = (
                Vec * boneList[i].length / 256 + boneList[boneList[i].parent].tail
            )
...
def ReadAnim(boneList, onefile, char_name):
    ...
    for i in range(0, charCount):
        ...
        alone.Address = (
            int.from_bytes(onefile.read(4), byteorder="little") + 4
        )  # offset just after the character count so we add 4 for the absolute offset
        alone.Size = int.from_bytes(onefile.read(4), byteorder="little")
        onefile.seek(4, 1)
        alone.hasTim = int.from_bytes(onefile.read(4), byteorder="little")
        if (
            alone.hasTim <= 0xD0000000
        ):  # if has<=0xd0000000 then it's a NPC with textures
...
def MCH_TO_BLEND():
    ...
    for i in range(0, header.BoneCount):
        Vec = Vector((0, 0, 1))
        rawbonelist[i].tail = (
            Vec * rawbonelist[i].length / 256 + rawbonelist[rawbonelist[i].parent].tail
        )
    ...
    for i in range(0, header.ObCount):
        ...
        for j in range(0, header.VCount):
            if (j >= skin.vertexFirst) and (
                j <= (skin.vertexFirst + skin.vertexCount - 1)
            ):
                grp.add([j], 1, "REPLACE")
            else:
                grp.add([j], 0, "REPLACE")
...
def BLEND_TO_MCH():
    ...
    print(
        "Exporting {}\nVcount:{} Fcount:{} UVcount:{} Vgroups:{} Bones:{}".format(
            header.char_name, Vcount, Fcount, UVcount, Vgroup_count, Bone_count
        )
    )
    ...
    newheader.FOffset = (
        newheader.TexAnimOffset + newheader.TexAnimSize
    )  # tex animation is at least 0x14 bytes
    newheader.Unk1Offset = newheader.FOffset + Fcount * 64  # a face is 64 bytes
    newheader.ObOffset = (
        newheader.Unk1Offset + newheader.Unk1Count * 32
    )  # unk 1 is 32 bytes
    newheader.AnimOffset = (
        newheader.ObOffset + newheader.ObCount * 8
    )  # a skin object is 8 bytes
    ...
    print(
        "NEW BONE OFFSET:{}\nNEW VERTS OFFSET:{}\nNEW TANIM OFFSET:{}\nNEW FACE OFFET:{}\nNEW UNK1 OFFSET:{}\nNEW SKINOB OFFSET:{}\nNEW ANIM OFFSET:{}\nNEW UNK2 OFFSET:{}\n".format(
            hex(newheader.BoneOffset),
            hex(newheader.VOffset),
            hex(newheader.TexAnimOffset),
            hex(newheader.FOffset),
            hex(newheader.Unk1Offset),
            hex(newheader.ObOffset),
            hex(newheader.AnimOffset),
            hex(newheader.Unk2Offset),
            "08x",
        )
    )
    ...
    outputfile.seek(0, 2)
    print(
        "REAL BONE OFFSET:{} ".format(
            hex(outputfile.tell() - header.ModelAddress), "08x"
        )
    )
    ...
    for bone in bonelist:
        if bone.name != "root":
            ...
            outputfile.write(
                ((bone.parent + 1) * 0x40).to_bytes(2, "little")
            )  # bone parent ID * 0x40. Why?
    ...
    Vorder = [
        [] for vg in range(newheader.ObCount)
    ]  # first index is the group ID, second index is the re-ordered vertex ID

    Vorder_total = 0
    rot_eul = Euler((0, 0, 0), "XYZ")
    me = char_ob.data
    vfound = [
        0 for i in range(len(char_ob.data.vertices))
    ]  # prevent doubles in vertex groups
    for vgroup in char_ob.vertex_groups:
        for vertID in range(0, len(char_ob.data.vertices)):
            try:
                ...
                if (vgroup.weight(vertID) > 0) and (
                    vfound[vertID] == 0
                ):  # groups needs to be perfectly independant.
                    vfound[vertID] = 1
    ...
    Vorder_total = 0
    for vgroup in char_ob.vertex_groups:
        bone = skl_ob.data.bones[
            vgroup.name
        ]  # beware : the bone ID in the skeleton is different from the original MCH
        boneID = -1
        for i in range(0, len(bonelist)):
        ...
        rot_eul = Euler(
            (
                -BoneRotations[boneID][0],
                -BoneRotations[boneID][1],
                -BoneRotations[boneID][2],
            ),
            "ZYX",
        )
    ...
    Vinvert = [
        0 for i in range(newheader.VCount)
    ]  # if vertID is global ID, order ID is the Vgroup ID of vertID, offset is the position of the Vgroup, then Vinvert[vertID]=orderID +offset is the re-ordered ID
    ...
    for vgroup in char_ob.vertex_groups:
        outputfile.write(
            (Vinvert[Vorder[vgroup.index][0]]).to_bytes(2, "little")
        )  # 1stvertex
        outputfile.write(
            len(Vorder[vgroup.index]).to_bytes(2, "little")
        )  # vertex count
        boneID = -1
        for i in range(0, len(bonelist)):
            if bonelist[i].name == vgroup.name:
                boneID = i
        outputfile.write(
            (boneID + 1).to_bytes(2, "little")
        )  # bone ID in base 1 for MCH
        outputfile.write(b"\x00" * 2)  # skip 2 bytes
